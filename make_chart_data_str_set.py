...
#    'stl_unordered_map': 'llvm unordered_map Apple clang version version 4.1 (tags/Apple/clang-421.11.66) (based on LLVM 3.1svn) std::tr1::unordered_map [const char*]',
	'stl_unordered_map_string': 'llvm unordered_map Apple clang version version 4.1 (tags/Apple/clang-421.11.66) (based on LLVM 3.1svn) std::tr1::unordered_map [std::string] (with length)',
...
#    'qt_qhash': 'Qt 4.5 QHash',
    ...
    'perl_hash': 'Perl 5.12.4 built for darwin-thread-multi-2level (C API) hash (with length options)',
...
#    'LuaHashMap51': 'Lua 5.1.5 (C API) table (via LuaHashMap)',
    'LuaHashMap52': 'Lua 5.2.1 (C API) table (via LuaHashMap) (with internalization & length options)',
}
